...
<main in:fade={{ duration: 400 }}>
  ...
  {:else if online && auth.signedIn}
    <!-- LOBBY -->
    <section class="lobby" in:fade={{ duration: 300 }}>
      <div class="acct">
        <h2 class="lobtitle">Lobby</h2>
        <span class="who">
          {#if auth.profile?.avatar}<img class="av" src={auth.profile.avatar} alt="" />{/if}
          {auth.profile?.name}{#if auth.isGuest}<span class="gtag">guest</span>{/if}
        </span>
      ...
      {#if openRooms.length === 0}
        <p class="empty">No open games. Create one above.</p>
      {:else}
        <ul class="rooms">
...
<style>
  main { max-width: 760px; margin: 0 auto; padding: clamp(2rem, 6vw, 4rem) 1.25rem 3rem; text-align: center; }
  .badge {
  ...
  .discord { border: 0; background: #5865f2; color: #fff; border-radius: 999px; padding: 0.6rem 1.3rem; font-weight: 700; cursor: pointer; font-size: 1rem; }
  /* login */
  .login {
    max-width: 420px;
    margin: 0 auto;
  ...
  .lobtitle { font-size: 1.2rem; margin: 0; }
  .acct { display: flex; align-items: center; gap: 0.8rem; flex-wrap: wrap; margin-bottom: 0.8rem; }
